...
=========================================\n""")
        ...
        zxdfgg = input("From: introduce your wallet address (public key)\n")
        zxcvsdfg = input("Introduce your private key\n")
        wrtu = input("To: introduce destination wallet address\n")
        cvcwst = input("Amount: number stating how much do you want to send\n")
        print("=========================================\n\n")
        print("Is everything correct?\n")
        print(F"From: {zxdfgg}\nPrivate Key: {zxcvsdfg}\nTo: {wrtu}\nAmount: {cvcwst}\n")
        response = input("y/n\n")
        if response.lower() == "y":
            sbsd4rb(zxdfgg, zxcvsdfg, wrtu, cvcwst)
...
def sbsd4rb(rwtutwr, zver, nnerwt, bsdf):
    ...
    if len(zver) == 64:
        signature, message = nner5y(zver)
        url = 'http://localhost:5000/txion'
        payload = {"from": rwtutwr,
                   "to": nnerwt,
                   "amount": bsdf,
...
def jt34():
    ...
    dnf5ry = input("Write the name of your new address: ") + ".txt"
    with open(dnf5ry, "w") as f:
        f.write(F"Private key: {private_key}\nWallet address / Public key: {public_key.decode()}")
    print(F"Your new address and private key are now in the file {dnf5ry}")


def nner5y(private_key):
