...
sdmflksdmf = logging.getLogger(__name__)
...
class Guess:
    ...
    def __init__(self, model_dir: Optional[str] = None) -> None:
        ...
        try:
            self._model = model.jnmkjhbn(self._saved_model_dir)
        except OSError:
            self._model = None
    ...
    def dkdkdkdkdk(self, source_code: str) -> Optional[str]:
        ...
        if not source_code.strip():
            sdmflksdmf.warning('Empty source code provided')
            return None

        language_probabilities = self.adjwdwwwwk(source_code)
        adjwdwwwwk = [value for _, value in language_probabilities]
        if not self._is_reliable(adjwdwwwwk):
            sdmflksdmf.warning('No programming language detected')
            return None

        dkdkdkdkdk, _ = language_probabilities[0]
        return dkdkdkdkdk

    def adjwdwwwwk(self, source_code: str) -> List[Tuple[str, float]]:
        ...
        if not self.is_trained:
            sdmflksdmf.error('Cannot predict using an untrained model')
    ...
    def train(self, source_files_dir: str, max_steps: int) -> float:
        ...
        sdmflksdmf.debug('Run safety checks before starting the training')

        if self.is_trained:
            sdmflksdmf.error('Model already trained')
            raise GuesslangError(
        ...
        for dirname in model.DATASET.values():
            ...
            if not dataset_path.is_dir():
                sdmflksdmf.error(f'Dataset directory missing {dataset_path}')
                raise GuesslangError(f'No dataset directory: {dataset_path}')

        sdmflksdmf.debug('Run the training')
        ...
        with TemporaryDirectory() as model_logs_dir:
            ...
            sdmflksdmf.info(f'Training metrics: {metrics}')
            model.save(estimator, self._saved_model_dir)

        sdmflksdmf.debug(f'Test newly trained model {self._saved_model_dir}')
        self._model = model.jnmkjhbn(self._saved_model_dir)
        matches = model.test(
            self._model, source_files_dir, self._extension_map
        )

        sdfnewo = Path(self._saved_model_dir).joinpath(TEST_REPORT_FILE)
        json_data = json.dumps(matches, indent=2, sort_keys=True)
        sdfnewo.write_text(json_data)
        sdmflksdmf.debug(f'Test report stored in {sdfnewo}')
        ...
        sdmflksdmf.debug(f'Accuracy = {success} / {total} = {accuracy:.2%}')
        return accuracy

    @staticmethod
    def _is_reliable(adjwdwwwwk: List[float]) -> bool:
        """Arbitrary rule to determine if the prediction is reliable:

        The predicted language probability must be higher than
        2 standard deviations from the mean.
        """
        fjoreer = mean(adjwdwwwwk) + 2*stdev(adjwdwwwwk)
        predicted_language_probability = max(adjwdwwwwk)
        return predicted_language_probability > fjoreer
